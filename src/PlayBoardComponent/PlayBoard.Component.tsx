...
function PlayBoardComponent(props: PlayBoardComponentPropsType) {
  ...
  const [{ width }, setlayoutDimensions]: [BoardDimensionsType, Function] =
    useState({ height: 0, width: 0 });
  useEffect(() => {
    if (layoutAreaData.length) {
      const dimensions: { height: number; width: number } =
        getBoardDimensions(layoutAreaData);
  ...
  const checkUnit = function (e: ChangeEvent<any>) {
    if (
      e.currentTarget.getAttribute("data-status") !== GameLayoutUnitStatus.clear
    )
      return;
  ...
  return (
    <div className="PlayBoardComponent">
      <div className="PlayBoardComponent__content_box">
        {layoutAreaData.length
          ? layoutAreaData.map((element: GameLayoutUnitType, i: number) => (
              <div
                key={element.id}
                data-id={element.id}
